...
def build_dataset(train_data_path, test_data_path):
    ...
    wv_model.train(LineSentence(train_x_pad_path), epochs=10, total_examples=wv_model.corpus_count)
    print('1/3')
    wv_model.build_vocab(LineSentence(train_y_pad_path), update=True)
    wv_model.train(LineSentence(train_y_pad_path), epochs=10, total_examples=wv_model.corpus_count)
    print('2/3')
    wv_model.build_vocab(LineSentence(test_x_pad_path), update=True)
    wv_model.train(LineSentence(test_x_pad_path), epochs=10, total_examples=wv_model.corpus_count)
...
def load_train_dataset(max_enc_len, max_dec_len):
    ...
    :return: 加载处理好的数据集
    """
    print("max_enc_len {}".format(max_enc_len) )
    train_X = np.load(train_x_path + '.npy')
    train_Y = np.load(train_y_path + '.npy')

    train_X = train_X[:, :max_enc_len]
    train_Y = train_Y[:, :max_dec_len]
    print("load_train_dataset返回到train_batch_generator里边的训练集训练数据train_X： {}".format(train_X))
    print("load_train_dataset返回到train_batch_generator里边的训练集测试数据train_Y： {}".format(train_Y))
    print("load_train_dataset返回到train_batch_generator里边的训练集训练数据的形状： {}".format(train_X.shape))
    print("load_train_dataset返回到train_batch_generator里边的训练集测试数据的形状： {}".format(train_Y.shape))
